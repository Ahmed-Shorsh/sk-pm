...
final class EvaluationRepository
{
    ...
    /**
 * Fetch all individual evaluations for admin view with optional department filter
 */
public function fetchIndividualEvaluations(string $month, ?int $deptId = null): array
{
    $sql = "
        SELECT 
            ie.month,
            ie.rating,
            ie.created_at as date_submitted,
            evaluator.name as evaluator,
            evaluatee.name as evaluatee,
            ii.indicator_name as indicator,
            d.dept_name
        FROM individual_evaluations ie
        JOIN users evaluator ON ie.evaluator_id = evaluator.user_id
        JOIN users evaluatee ON ie.evaluatee_id = evaluatee.user_id
        JOIN individual_indicators ii ON ie.indicator_id = ii.indicator_id
        JOIN departments d ON evaluatee.dept_id = d.dept_id
        WHERE ie.month = :month
    ";
    
    $params = [':month' => $month];
    
    if ($deptId !== null) {
        $sql .= " AND evaluatee.dept_id = :dept";
        $params[':dept'] = $deptId;
    }
...
/**
 * Check if user has already submitted evaluations for the given month
 */
public function hasUserSubmittedForMonth(int $evaluatorId, string $month): bool
{
    $count = (int)$this->selectScalar("
        SELECT COUNT(DISTINCT evaluatee_id)
        FROM individual_evaluations
        WHERE evaluator_id = :evaluator
        AND month = :month
    ", [':evaluator' => $evaluatorId, ':month' => $month]);
