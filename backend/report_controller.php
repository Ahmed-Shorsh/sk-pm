...

{
...

    {
        ...
        foreach ($people as $p) {
            ...
            // Compute individual's score (% of goal) – include manager indicators if applicable
            $cond   = ($p['role_id'] == 2) 
            ...
            $indRows = $this->select(
                ...
                [':uid' => $p['user_id'], ':m' => $month]
            );
            $indPerf = 0.0;
            if (count($indRows) > 0) {
                $totalPerc = 0.0;
                foreach ($indRows as $r2) {
                    $totalPerc += ($r2['avg_rating'] / $r2['default_goal']) * 100;
                }
                $indPerf = $totalPerc / count($indRows);
            }
                $final = round(
                $deptScore * $this->deptWeight +
                $indPerf   * $this->indWeight,
                2
            );
            ...
            $rows[] = [
                ...
                'individual_score'=> round($indPerf, 2),
...

    {
             ...
             $u = $this->selectOne(
             'SELECT name, dept_id, role_id FROM users WHERE user_id = :id',
        ...
        $deptScore = $this->deptPlanScore((int)$u['dept_id'], $month);
                ...
                // Calculate individual performance (%) across all relevant indicators
                $catCondition = ($u['role_id'] == 2)
                    ? "ind.category IN ('individual','manager')" 
                    : "ind.category = 'individual'";
                $indRows = $this->select(
                    "SELECT ind.default_goal, AVG(ev.rating) AS avg_rating
                     FROM individual_evaluations ev
                     JOIN individual_indicators ind ON ev.indicator_id = ind.indicator_id
                     WHERE ev.evaluatee_id = :u AND ev.month = :m AND $catCondition
                     GROUP BY ind.indicator_id",
                    [':u' => $userId, ':m' => $month]
                );
                $indPerf = 0.0;
                if (count($indRows) > 0) {
                    $totalPerc = 0.0;
                    foreach ($indRows as $r) {
                        $totalPerc += ($r['avg_rating'] / $r['default_goal']) * 100;
                    }
                    $indPerf = $totalPerc / count($indRows);
                }
                $final     = round(
