...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'send') {
  ...
  try {
    ...
    flashMessage('Reminder sent to ' . $u['name'] . '.', 'success');
  } catch (Exception $e) {
...
$users       = $deptFilter === 'all' ? $allUsers : array_filter($allUsers, fn($u) => $u['dept_id'] == $deptFilter);
      ...
      <div class="col-md-4">
        ...
        <select class="form-select" name="dept_id" onchange="this.form.submit()">
          ...
          <?php foreach ($departments as $d): ?>
            <option value="<?= $d['dept_id'] ?>" <?= $d['dept_id'] == $deptFilter ? 'selected' : '' ?>><?= htmlspecialchars($d['dept_name']) ?></option>
          <?php endforeach; ?>
        </select>
      </div>
    </form>
      ...
      <table class="table table-bordered align-middle">
        ...
        <tbody>
          <?php foreach ($users as $u): ?>
            <?php $open = isWindowOpen($u); ?>
            <tr <?= $open ? '' : 'class="table-warning"' ?>>
              <td><?= htmlspecialchars($u['name']) ?></td>
              <td><?= htmlspecialchars($u['role_name']) ?></td>
              <td>
  <?= $u['rating_window_days'] === null 
        ? 'Default (' . $globalDays . ')' 
        : $u['rating_window_days'] ?>
              ...
              <td class="text-center">
                <button class="btn btn-sm btn<?= $open ? '-primary' : '-secondary' ?>" data-bs-toggle="modal" data-bs-target="#msg<?= $u['user_id'] ?>" <?= $open ? '' : 'disabled' ?>>Message</button>
              </td>
            </tr>
          <?php endforeach; ?>
        </tbody>
      ...
      <div class="modal-dialog">
        <form method="post" class="modal-content">
          ...
          <div class="modal-header">
            <h5 class="modal-title">Send Reminder — <?= htmlspecialchars($u['name']) ?></h5><button class="btn-close" data-bs-dismiss="modal"></button>
          </div>
          <div class="modal-body">
            <label class="form-label">Message</label>
            <textarea name="message" rows="6" class="form-control"><?= htmlspecialchars($default) ?></textarea>
            <?php if (!$open): ?><div class="alert alert-warning mt-3">Window closed for this user.</div><?php endif; ?>
          </div>
          <div class="modal-footer">
            <button class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
            <button class="btn btn-primary" <?= $open ? '' : 'disabled' ?>>Send</button>
          </div>
        </form>
      </div>
    </div>
