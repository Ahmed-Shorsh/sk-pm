...
function deptMembers(PDO $pdo, int $dept): array
{
  $stm = $pdo->prepare('SELECT user_id,name FROM users WHERE dept_id = :d AND active = 1');
  $stm->execute([':d' => $dept]);
...
$departments = ($roleId === ROLE_ADMIN)
  ? $deptRepo->fetchAllDepartments()
  : [$deptRepo->getDepartmentById($viewer['dept_id']) ?? ['dept_id' => $viewer['dept_id'], 'dept_name' => '']];

$selectedDept  = (int)($_GET['dept_id'] ?? $departments[0]['dept_id']);
$selectedMonth = $_GET['month'] ?? ($months[0] ?? date('Y-m-01'));
$selectedUser  = isset($_GET['user_id']) && $_GET['user_id'] !== '' ? (int)$_GET['user_id'] : null;
$viewMode      = ($_GET['view'] ?? 'mgr') === 'dept' ? 'dept' : 'mgr';

if ($roleId === ROLE_ADMIN && isset($_GET['export_pdf']) && $selectedUser) {
...
<table>
  <thead>
    <tr><th>Block</th><th>Weight</th><th>Contrib.</th></tr>
  </thead>
  <tbody>
    <tr>
      <td>Dept Plan</td>
    ...
    <tr>
      ...
      <th><?= $sum['final_score'] ?> %</th>
          ...
          $dom->setPaper('A4');
          $dom->render();
          $dom->stream(sprintf('sk-pm_%s_%s.pdf', preg_replace('/\s+/', '_', $sum['name']), str_replace('-', '', $selectedMonth)), ['Attachment' => true]);
          exit;
        }
        ...
        if ($selectedUser) {
          ...
          if ($viewMode === 'dept') {
            ...
            $rows = array_map(fn($r) => ['name' => $r['name'], 'dept' => $r['dept_score'], 'ind' => $r['individual_score'], 'mgr' => '-', 'fin' => $r['final_score']], $tmp);
          } else {
            $tmp = $reportRepo->deptEvalSummary($selectedDept, $selectedMonth);
            $rows = array_map(fn($r) => ['name' => $r['name'], 'dept' => '-', 'ind' => $r['ind_avg'], 'mgr' => $r['mgr_avg'], 'fin' => $r['final_score']], $tmp);
          }
        }

        if ($selectedUser) {
          [$avgDept, $avgInd, $avgMgr, $avgFin] = [$rows[0]['dept'], $rows[0]['ind'], $rows[0]['mgr'], $rows[0]['fin']];
        } else {
          $avgInd = round(array_sum(array_column($rows, 'ind')) / max(count($rows), 1), 2);
          $avgMgr = $viewMode === 'mgr' ? round(array_sum(array_filter(array_column($rows, 'mgr'), 'is_numeric')) / max(count($rows), 1), 2) : '-';
          $avgDept = $viewMode === 'dept' ? $reportRepo->deptPlanScore($selectedDept, $selectedMonth) : '-';
...
<body class="bg-light font-serif">
  <div class="container py-4">
    ...
    <form class="row g-3 align-items-end mb-4" method="get">
      <?php if ($roleId === ROLE_ADMIN): ?>
        <div class="col-md-3"><label class="form-label">Department</label>
          <select name="dept_id" class="form-select" onchange="this.form.submit()">
            <?php foreach ($departments as $d): ?><option value="<?= $d['dept_id'] ?>" <?= $d['dept_id'] === $selectedDept ? 'selected' : '' ?>><?= htmlspecialchars($d['dept_name']) ?></option><?php endforeach; ?>
          </select>
        </div><?php endif; ?>
      <div class="col-md-3"><label class="form-label">Month</label>
        <select name="month" class="form-select" onchange="this.form.submit()">
          <?php foreach ($months as $m): ?><option value="<?= $m ?>" <?= $m === $selectedMonth ? 'selected' : '' ?>><?= date('F Y', strtotime($m)) ?></option><?php endforeach; ?>
        </select>
      </div>
      <div class="col-md-3"><label class="form-label">User</label>
        <select name="user_id" class="form-select" onchange="this.form.submit()">
          <option value="">All</option>
          <?php foreach (($deptRepo->fetchDepartmentMembers($selectedDept) ?? deptMembers($pdo, $selectedDept)) as $mem): ?>
            <option value="<?= $mem['user_id'] ?>" <?= $mem['user_id'] === $selectedUser ? 'selected' : '' ?>><?= htmlspecialchars($mem['name']) ?></option>
          <?php endforeach; ?>
        </select>
      </div>
      <div class="col-md-2"><label class="form-label">View</label>
      ...
      <?php if ($roleId === ROLE_ADMIN && $selectedUser): ?><div class="col-auto"><button class="btn btn-secondary" name="export_pdf" value="1">Export PDF</button></div><?php endif; ?>
    </form>
    <div class="row mb-4">
      <div class="col-md-3">
      ...
      <?php if ($viewMode === 'mgr'): ?><div class="col-md-3">
          <div class="card text-center shadow-sm">
            <div class="card-body">
              <h6>Avg Manager</h6>
      ...
      <table class="table table-striped table-bordered align-middle">
        <thead class="table-dark">
          <tr>
            <th>Name</th>
            <th>Dept</th>
            <th>Ind</th><?php if ($viewMode === 'mgr'): ?><th>Mgr</th><?php endif; ?><th>Final</th>
          </tr>
        </thead>
        <tbody>
          <?php foreach ($rows as $r): ?><tr>
              ...
              <td><?= $r['ind'] ?>%</td>
              <?php if ($viewMode === 'mgr'): ?><td><?= is_numeric($r['mgr']) ? $r['mgr'] . '%' : '-' ?></td><?php endif; ?>
              <td><?= $r['fin'] ?>%</td>
            </tr><?php endforeach; ?>
        </tbody>
      </table>
